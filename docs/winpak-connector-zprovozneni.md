...
| `WIN-PAK AddUpdateCard: Number of parameters specified does not match the expected number` při uložení karty | skutečná metoda má víc parametrů než příručka (do v1.12.7) | od v1.12.8 konektor doplní chybějící parametry podle typové informace objektu a stav zápisu vyhodnotí; když by nesoulad zůstal, hláška vypíše skutečnou signaturu — pošlete ji |
| `WIN-PAK DeviceID: Method 'System.String.DeviceID' not found` na stránce Systém (a obecně `Method 'System.String.X' not found`) | WIN-PAK vrátil seznam názvů nebo čísel místo objektů (do v1.12.8) | od v1.12.9 se prostá hodnota v seznamu obaluje a mapování dostane název nebo id; stránka Systém načítá části zvlášť |
