...
# .NET 10 SDK (build v updateru) — přes dnf, s fallbackem na dotnet-install
...
# První build a start
$SUDO -u acs /opt/acs/bin/acs-updater.sh || true
$SUDO systemctl enable --now acs-web acs-updater.timer
echo "--- Stav: ---"
